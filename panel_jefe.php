...
// ✅ Obtener docentes de esta carrera (por asignaciones)
$stmt1 = $conn->prepare("
    SELECT DISTINCT u.id, u.nombre, u.email
    FROM asignaciones a
    JOIN tipodeusuarios u ON u.id = a.maestro_id
    WHERE a.carrera_id = ? AND u.rol = 'maestro' AND u.estado = 'activo'
    ORDER BY u.nombre
");
...
// ✅ Estadísticas corregidas con asignaciones
$stmt3 = $conn->prepare("
    SELECT u.id as docente_id, u.nombre as docente, COUNT(f.id) as total_practicas
    FROM asignaciones a
    JOIN tipodeusuarios u ON a.maestro_id = u.id
    LEFT JOIN fotesh f ON f.Maestro_id = u.id AND f.Materia_id = a.materia_id AND a.carrera_id = ?
    WHERE u.rol = 'maestro' AND u.estado = 'activo' AND a.carrera_id = ?
    GROUP BY u.id, u.nombre
    ORDER BY total_practicas DESC
");
$stmt3->bind_param("ii", $carrera_id, $carrera_id);
$stmt3->execute();
                    ...
                    <tbody>
                        <?php while ($docente = $docentes->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                <a href="ver_fotesh.php?docente_id=<?= $docente['id'] ?>&carrera_id=<?= $carrera_id ?>" class="btn btn-sm btn-info">FO-TESH</a>
                                <a href="ver_reportes_docente.php?docente_id=<?= $docente['id'] ?>&carrera_id=<?= $carrera_id ?>" class="btn btn-sm btn-secondary">Reportes</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                    ...
                    <tbody>
                        <?php while ($row = $estadisticas->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="detalle_estadisticas_docente.php?docente_id=<?= $row['docente_id'] ?>&carrera_id=<?= $carrera_id ?>" class="btn btn-sm btn-primary">Ver Detalle</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
